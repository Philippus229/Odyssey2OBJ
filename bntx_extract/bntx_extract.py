...
def saveTextures(textures, out_folder):
    for tex in textures:
        if tex.format in formats and tex.numFaces < 2:
            ...
            if (tex.format >> 8) in ASTC_formats:
                ...
                with open(os.path.join(out_folder, tex.name + ".astc"), "wb+") as output:
                    output.write(outBuffer)
            else:
                hdr = dds.generateHeader(1, tex.width, tex.height, format_, list(reversed(tex.compSel)), size, (tex.format >> 8) in BCn_formats)
                with open(os.path.join(out_folder, tex.name + ".dds"), "wb+") as output:
...
def extract_textures(filename, output_folder):
    with open(filename, "rb") as inf:
        inb = inf.read()
    textures = readBNTX(inb)
    saveTextures(textures, output_folder)
    return 1
